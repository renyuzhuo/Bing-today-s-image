...
def downloadImage():
    ...
    for imageObjTemp in imagesObj:
        ...
        imageWebUrl = imgUrl
        imgUrl = imgUrl.replace('http://bimgs.plmeizi.com/images/bing/2017/', '').replace(
            'http://bimgs.plmeizi.com/images/bing/2016/', '').replace('http://s.cn.bing.net/az/hprichbg/rb/', '')
...
def getImageFromBing():
    ...
    for bingImageObj in jsonObj['images']:
        ...
        if int(bingImageObj['enddate']) > imageData:
            oneDay = OneDay(bingImageObj['copyright'] + ' (' + bingImageObj['enddate'] + ')',
                            'http://www.bing.com' + bingImageObj['url'], bingImageObj['enddate'])
            img = requests.get('http://www.bing.com' + bingImageObj['url'])
            imgUrl = bingImageObj['url'].replace('th?id=OHR.', '').replace('&rf=LaDigue_1920x1080.jpg&pid=hp',
                                                                           '').replace(
                '&rf=NorthMale_1920x1080.jpg&pid=hp', '').replace('/az/hprichbg/rb/', '')
